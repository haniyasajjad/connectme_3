...
class Screen12 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        followersCount.setOnClickListener {
            ...
            intent.putExtra("userName", currentUser.text)
    ...
    private fun fetchFollowing() {
        val url = "${baseUrl}get_following.php?user_id=$userId"
        val request = JsonObjectRequest(
            Request.Method.GET, url, null,
            { response ->
                Log.d("Screen11", "Following response: $response")
                if (response.getString("status") == "success") {
                    userList.clear()
                    ...
                    Log.d("Screen11", "Following list size: ${userList.size}")
                    if (userList.isEmpty()) {
    ...
    private fun searchFollowing(query: String) {
        val url = "${baseUrl}search_follow.php?user_id=$userId&type=following&query=$query"
        val request = JsonObjectRequest(
            Request.Method.GET, url, null,
            { response ->
                Log.d("Screen11", "Search following response: $response")
                if (response.getString("status") == "success") {
                    userList.clear()
                    val users = response.getJSONArray("users")
                    for (i in 0 until users.length()) {
                    ...
                    Log.d("Screen11", "Search following list size: ${userList.size}")
                    adapter.notifyDataSetChanged()
                } else {
                    Toast.makeText(this, "Error: ${response.getString("message")}", Toast.LENGTH_SHORT).show()
                }
            ...
            { error ->
                Log.e("Volley", "Error searching following: ${error.message}")
                Toast.makeText(this, "Network error: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        )
        queue.add(request)
